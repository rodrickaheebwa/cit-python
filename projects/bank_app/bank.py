...
class Bank:
    ...
    def create_account_number(self):
        ...
        # AC+last 4 digits of phone number+current date+random number and account number max length 15
        # e.g. AC13592022091972346589137
        account_number = 'AC' + \
            self.phone[-4:] + datetime.now().strftime('%Y%m%d') + \
            str(secrets.randbits(15))
        return account_number

    # create a connection to the database we will use
    def connect_db(self):
        try:
    ...
    def create_tables(self):
        ...
        conn.commit()
        conn.close()
        print("Bank database and tables created!")
    ...
    def deposit(self, amount):
        ...
        print("You have deposited UGX {}".format(str(amount)))
        self.check_balance()
